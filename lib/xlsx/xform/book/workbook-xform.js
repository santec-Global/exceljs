...
class WorkbookXform extends BaseXform {
  ...
  render(xmlStream, model) {
    ...
    this.map.definedNames.render(xmlStream, model.definedNames, model.passthroughDefinedNames);
  ...
  reconcile(model) {
    ...
    _.each(model.definedNames, definedName => {
      if (definedName.name === '_xlnm.Print_Area') {
        ...
        if (worksheet) {
          ...
          // Use the passthrough to handle print areas since they cannot always be handled properly by exceljs due to the fact that they are not stored in the same way as they are in the xlsx file
          // We don't need to actually touch the print areas for teh way we use exceljs in XNS
          // This passthrough only applies for sheets without a defined print area
          if(worksheet.pageSetup.printArea) {
            worksheet.pageSetup.printArea = `${worksheet.pageSetup.printArea}&&${range.dimensions}`;
          }
          else {
            passthroughDefinedNames.push(definedName);
          }
        }
    ...
    model.passthroughDefinedNames = passthroughDefinedNames;
